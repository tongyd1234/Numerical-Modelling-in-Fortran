...
!   integer, parameter :: dp = selected_real_kind(15, 307)
   real(kind=dp)::a_dif,a_adv,total_time,h,k,dt_dif,dt_adv,time,Ray_Num,dt,alpha
   real(kind=dp)::S_rms,T_rms,W_rms,rhs_rms,t1
   real(kind=dp),allocatable::T(:,:),S(:,:),W(:,:),vx(:,:),vy(:,:),rhs(:,:),v_grad(:,:)

   namelist /inputs/ L,total_time,k,a_dif,a_adv,nx,ny,Ray_Num
   open(1,file='parameters_conv.txt',status='old')
   read(1,inputs)
   close(1)
   print*,'My input parameters are:'
   write(*,inputs)
   allocate(T(nx,ny),S(nx,ny),W(nx,ny),vx(nx,ny),vy(nx,ny),rhs(nx,ny),v_grad(nx,ny))
!initialise parameters
   h=1.0/(ny-1)
   dt_dif=a_dif*h*h/k
   alpha=1
   time=0
!initialise allocatalbe arryas
  do 
     write(*,*) 'Enter 1 for initial source and 2 for random source: '
     read(*,*) a
     if ((a>=1).and.(a<=2)) then
        exit
     end if
  ...
  if (a==1) then
   print*,'You are calling the spike source'
   T(nx/2+1,ny/2+1)=1./h**2
  end if
  if (a==2) then
   print*,'You are calling the random source'
...
!Boundary condition for T
   ...
   rhs_rms=sqrt(sum(rhs*rhs)/(nx*ny))
   W_rms=rhs_rms/10 !set w_rms to make sure the loop would run
   do
      if (W_rms/rhs_rms<=10.**(-3)) exit
      W_rms=Vcycle_2DPoisson(W,rhs,h)
   end do
...
!Determine dt
   ...
   open(2,file='exercise7_1.dat')
      do j=1,ny
      write(2,'(1000(1pe13.5))') S(:,j)
      end do
   close(2)
   open(3,file='exercise7_2.dat')
      do j=1,ny
      write(3,'(1000(1pe13.5))') T(:,j)
      end do
   close(3)
   deallocate(T,S,W,vx,vy,rhs,v_grad)
   call cpu_time(t1)
