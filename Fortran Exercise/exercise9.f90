...
!---------------
real(dp) function iteration_2DPoisson_modified(u,f,h,alpha,c)
implicit none
real(kind=dp),intent(in)::f(:,:)
real(kind=dp),intent(in)::h,alpha,c
real(kind=dp),intent(inout)::u(:,:)
real(kind=dp),dimension(size(f,dim=1),size(f,dim=2))::res
integer::i,j,nx,ny
nx=size(f,dim=1)
...
do i=2,nx-1
   do j=2,ny-1
      res(i,j)=(u(i-1,j)+u(i+1,j)+u(i,j+1)+u(i,j-1)-(4+c*h*h)*u(i,j))/h**2-f(i,j)
      u(i,j)=u(i,j)+alpha*res(i,j)*h*h/(4+c*h*h)
   end do
end do
iteration_2DPoisson_modified=sqrt(sum(res**2)/(nx*ny))
end function iteration_2DPoisson_modified
!---------------
...
!-------------------
subroutine residue_2DPoisson_modified(u,f,h,res,c)
implicit none
real(kind=dp),intent(in)::u(:,:),f(:,:)
real(kind=dp),intent(in)::h,c
real(kind=dp),dimension(size(f,dim=1),size(f,dim=2)),intent(out)::res
integer::i,j,nx,ny
nx=size(u,dim=1)
ny=size(u,dim=2)
...
res(:,1)=0
res(:,ny)=0
end subroutine residue_2DPoisson_modified
!---------------
...
!---------------
  recursive function Vcycle_2DPoisson(u_f,rhs,h,c,beta,Tflag,Nflag) result (resV)
    implicit none
    real resV
    logical,intent(in)::Tflag
    real(kind=dp),intent(inout):: u_f(:,:)  ! arguments
    real(kind=dp),intent(in)   :: rhs(:,:),h,c,beta
    real(kind=dp),allocatable  :: res_c(:,:),corr_c(:,:),res_f(:,:),corr_f(:,:)    
    integer		:: nx, ny, nxc,nyc, i ,Nflag ! local variables
...
!--------------------Original Code-----------------------
    ...
    if (min(nx,ny)>5) then  ! not the coarsest level
       ...
       corr_c = 0.  
       res_rms = Vcycle_2DPoisson(corr_c,res_c,h*2,c,beta,Tflag,Nflag) ! *RECURSIVE CALL*
       !print*,res_rms
...
!-------------------------------Calculate the modified one------------------------
    if (beta>0) then

    if(nxc*2-1 /= nx .or. nyc*2-1 /= ny) stop "ERROR: not possible to divide grid by 2"
    if ((Tflag) .and. (nx<Nflag)) then
      u_f(:,ny)=0
      u_f(:,1)=0
      u_f(1,:)=u_f(2,:)
      u_f(nx,:)=u_f(nx-1,:)
...
!print*,'Yandong3'
    ...
    if (min(nx,ny)>5) then  ! not the coarsest level
       ...
       !---------- take 2 iterations on the fine grid--------------
       res_rms = iteration_2DPoisson_modified(u_f,rhs,h,alpha,c) 
       res_rms = iteration_2DPoisson_modified(u_f,rhs,h,alpha,c)

       !---------- restrict the residue to the coarse grid --------
       call residue_2DPoisson_modified(u_f,rhs,h,res_f,c) 
       call restrict(res_f,res_c)
       !print*,res_rms
       !---------- solve for the coarse grid correction -----------
       corr_c = 0.  
...
!-----------------
subroutine w_grad_cal(vx,vy,W,h,nx,ny,w_grad)
implicit none
integer,intent(in)::nx,ny
real(kind=db),dimension(nx,ny),intent(in)::vx,vy,W
real(kind=db),intent(in)::h
real(kind=db),dimension(nx,ny)::vx_dt,vy_dt
real(kind=db),dimension(nx,ny),intent(out)::w_grad
integer::i,j
do i=2,nx-1
do j=2,ny-1
if (vx(i,j)>=0) then
vx_dt(i,j)=vx(i,j)*(W(i,j)-W(i-1,j))/h
end if
if (vx(i,j)<0) then
vx_dt(i,j)=vx(i,j)*(W(i+1,j)-W(i,j))/h
end if
if (vy(i,j)>=0) then
vy_dt(i,j)=vy(i,j)*(W(i,j)-W(i,j-1))/h
end if
if (vy(i,j)<0) then
vy_dt(i,j)=vy(i,j)*(W(i,j+1)-W(i,j))/h
end if
end do
end do
do i=1,nx,nx-1
...
program  diffusion
   ...
   implicit none
   integer::nx,ny,i,j,L,a,Nflag
   real(kind=dp)::a_dif,a_adv,total_time,h,dt_dif,dt_adv,time,Ray_Num,dt,alpha,Pr
   real(kind=dp)::S_rms,T_rms,W_rms,rhs_rms,t1,ct,cw,beta,c,ft_rms,fw_rms,b
   real(kind=dp),allocatable::T(:,:),S(:,:),W(:,:),vx(:,:),vy(:,:),rhs(:,:),v_grad(:,:),w_grad(:,:),ft(:,:),fw(:,:)
   logical::Tflag
   namelist /inputs/ L,total_time,a_dif,a_adv,nx,ny,Ray_Num,Pr,beta
   open(1,file='parameters_implicit.txt',status='old')
   read(1,inputs)
   close(1)
   print*,'My input parameters are:'
   write(*,inputs)
   allocate(T(nx,ny),S(nx,ny),W(nx,ny),vx(nx,ny),vy(nx,ny),rhs(nx,ny),v_grad(nx,ny),w_grad(nx,ny),ft(nx,ny),fw(nx,ny))
...
!Boundary condition for W
   ...
   do
      ...
      S_rms=Vcycle_2DPoisson(S,W,h,c,b,Tflag,Nflag)
!      print*,S_rms
   end do
!Boundary condition for W
   S(1,:)=0
...
!Determine dt
   ...
   call w_grad_cal(vx,vy,W,h,nx,ny,w_grad)
   ...
   if (beta>0) then
          ct=1./(dt*beta)
          ft=-1./(dt*beta)*(T+dt*((1-beta)*secdderivative(T,h)-v_grad))
             ft_rms=sqrt(sum(ft*ft)/(nx*ny))
             Tflag=.True.
             T_rms=ft_rms/10.  !Set T_rms to make sure the loop would run
                do
                   if (T_rms/ft_rms<=10.**(-3)) exit
                   T_rms=Vcycle_2DPoisson(T,ft,h,ct,beta,Tflag,Nflag)
!                  print*,T_rms
                end do
          cw=1./(dt*beta*Pr)
          fw=-1./(dt*beta*Pr)*(w+dt*((1-beta)*Pr*secdderivative(W,h)-w_grad-Pr*rhs))
             fw_rms=sqrt(sum(fw*fw)/(nx*ny))
             Tflag=.False.
             W_rms=fw_rms/10.  !Set W_rms to make sure the loop would run
                do
                   if (W_rms/fw_rms<=10.**(-3)) exit
                   W_rms=Vcycle_2DPoisson(W,fw,h,cw,beta,Tflag,Nflag)
                end do
          time=time+dt
   end if
end do
print*,'Timestep of advection',dt_adv
print*,'Timestep of diffusion',dt_dif
print*,'Timestep',dt
   open(1,file='exercise9_1.dat')
      do j=1,ny
      write(1,'(1000(1pe13.5))') W(:,j)
      end do
   close(1)
   open(2,file='exercise9_2.dat')
      do j=1,ny
      write(2,'(1000(1pe13.5))') S(:,j)
      end do
   close(2)
   open(3,file='exercise9_3.dat')
      do j=1,ny
      write(3,'(1000(1pe13.5))') T(:,j)
      end do
   close(3)
   deallocate(T,S,W,vx,vy,rhs,v_grad,w_grad,ft,fw)
   call cpu_time(t1)
